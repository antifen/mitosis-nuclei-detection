...
for i in range(len(HPFs_list)):
    ...
    HPF_result2 = []
    HPF_tmp_array2 = np.zeros(shape=(len(predict_boxes2), 5), dtype='float32')
    for p in range(HPF_tmp_array2.shape[0]):
        HPF_tmp_array2[p] = np.array([predict_boxes2[p][0], predict_boxes2[p][1], predict_boxes2[p][2],
                                     predict_boxes2[p][3], predict_scores2[p]])
    HPF_result2.append(HPF_tmp_array2)
    model.show_result(HPF_path, HPF_result2, out_file=test_results_HPF_path + "/" + HPFs_list[i])
